...
function normalizeText(input: string): string {
  return (input || "")
    ...
    .replace(/\\_/g, " ")
    .replace(/\\href\{[^}]*\}\{([^}]*)\}/g, " $1 ")
    .replace(/\\(?:textbf|textit|emph|underline|small)\{([^}]*)\}/g, " $1 ")
...
function buildCorrectionMessage(validation: ValidationResult): string {
  ...
  return [
    ...
    "Use the available one-page budget aggressively when supported by source evidence; avoid a sparse final page.",
    ...
    "- if the page is too sparse, add truthful bullets by splitting or sharpening existing source evidence",
